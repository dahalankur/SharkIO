...
class Player():
    ...
    """
    def __init__(self, name = "Example", unique_id = 0):
        """
        ...
        self.__chunk = Chunk(int_x, int_y, PLAYER_RADIUS,
                             self.__get_random_color(), unique_id, 
                             PLAYER_VELOCITY, 0)
        self.__name = name
        self.__unique_id = unique_id
        self.__shark_as_string = image.tostring(self.generate_shark\
                                 (self.__unique_id), 'RGB')
      
    def __get_random_color(self):
        """
        Returns a random color instance for the player
        """
        valid = False
        while not valid:
            valid = True
            ...
            if (r > 220 and g > 220 and b > 220): valid = False
            # MAKE SURE NOT GREEN
            if (r < 25 and g > 220 and b < 25): valid = False
            # MAKE SURE NOT BLUE
            if (r < 25 and g < 25 and b > 220): valid = False
        return Color(r, g, b)
    ...
    def generate_shark(self, id):
        ...
        # get the shark image
        shark_image_name = "../shark_images/" + \
                           shark_images_dir[id % num_sharks]
    ...
    def get_shark_as_image(self, dimensions):
        ...
        player.
        """
        return transform.scale(image.fromstring(self.__shark_as_string, 
                              (100, 100), 'RGB'), dimensions)

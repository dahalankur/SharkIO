...
def recv_data(sock):
    """
    Receives the whole data sent via the socket sock
    """
...
def recvall(sock, n):
    """
    Given number of bytes n, receives n bytes sent via the socket sock
    """
    data = bytearray()
    while len(data) < n:
...
def main():
                ...
                try:
                    with state_lock:
                        ...
                        serialized_data = pickle.dumps(\
                                          (gameboard.get_objects_for_player(id),
                                           gameboard.get_players(),
                                           gameboard.get_player(id)))
                    ...
                    if not data:
                        continue
                    chunk = pickle.loads(data) # receive updated chunk
                    if not chunk:
                        continue
                    
                    gameboard.get_player(id).set_chunk(chunk)
                    
                    # Functions handle data races
                    check_player_collisions()
                    check_other_collisions(gameboard.get_player(id))
                except KeyError:
                    # means the player died because gameboard.get_player(id) 
                    # will crash, so respawn after 2 seconds
                    time.sleep(2)
                    player = Player(name=name.decode('utf-8'), unique_id=id)
                    gameboard.add_player(player)
                except BrokenPipeError:
                    # player quit their client and the connection broke
                    break
                except ConnectionResetError:
                    # player quit their client and the connection was reset
                    break
    ...
    def check_player_collisions():
        ...
        """
        players_sorted_by_score = sorted(gameboard.get_players().values(),
        ...
        for other in objects_array:            
            if chunk.is_colliding(other):
                if other.is_virus():
                    ...
                    if (chunk.get_radius() - size_change) > \
                                                        PLAYER_MINIMUM_RADIUS:
                        chunk.set_radius(chunk.get_radius() - size_change)
    ...
    def begin_object_generation():
        """
        Periodically generate game objects on the gameboard
        """
        iterations = 1
        while True:
                ...
                if len(gameboard.get_objects()) < MAX_GAME_OBJECTS:
                    ...
                    if num_food < MAX_FOOD_IN_GAME:
                        for _ in range(num_players):
                            gameboard.add_object(GameObject(randint(0, 
                                                         gameboard.get_width()),
                                                         randint(0, 
                                                         gameboard.get_height()\
                                                         ), FOOD_RADIUS, BLUE, 
                                                         'food', uuid.uuid4()))
                    if num_virus < MAX_VIRUS_IN_GAME:
                        if iterations % FOOD_GENERATION_ITERATIONS == 0:
                            virus_radius = randint(VIRUS_RADIUS_MIN, 
                                                   VIRUS_RADIUS_MAX)
                            gameboard.add_object(GameObject(randint(0, 
                                                        gameboard.get_width()),
                                                        randint(0, 

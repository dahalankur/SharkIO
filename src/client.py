...
from constants import HOST, PORT, BOARD_HEIGHT, BOARD_WIDTH, DISPLAY_HEIGHT, \
    DISPLAY_WIDTH, RED
...
def init_pygame():
    ...
    return world, clock, font, font2, display
...
def run_client():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        world, clock, font, font2, display = init_pygame()
        world.fill(WHITE)
        ...
        while running:
            ...
            for p in players.values():
                p_chunk = p.get_chunk()
                pygame.draw.circle(world, p_chunk.get_color(), p_chunk.get_pos(), p_chunk.get_radius())
            
             # render score UI
            text = font.render("Score: " + str(int(player.get_score())), True, (255, 0, 0))
            world.blit(text, (30, 700))

            # render leaderboard
            loc = 20
            leader_txt = 'Leaderboard'
            text2 = font2.render(leader_txt, True, (255, 0, 0))
            world.blit(text2, (1220, loc))
